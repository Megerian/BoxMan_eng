...
public class myExport extends Activity implements myGifMakeFragment.GifMakeStatusUpdate{
		 ...
		 bt_OK.setOnClickListener(new View.OnClickListener() {
			 ...
			 public void onClick(View v) {
				 if (cb_File.isChecked()) {  //导出到文档
					 ...
					 if (file.exists()) {
						 new AlertDialog.Builder(myExport.this, AlertDialog.THEME_HOLO_DARK).setNegativeButton(getString(R.string.cancel), null)
								 ...
								 .setMessage(getString(R.string.a_file_with_the_same_name_already_exist_overwrite)+ "\n" + my_Name).setCancelable(false)
								 .show(); //自设关卡已存在
					 } else {
						 saveFile(et_Action.getText().toString(), my_Name);
					 }
	...
	private void saveFile(String str, String fn) {
		try{
			...
			new AlertDialog.Builder(myExport.this, AlertDialog.THEME_HOLO_DARK).setNegativeButton(getString(R.string.confirm), new DialogInterface.OnClickListener() {
						@Override
...
//			MyToast.showToast(this, "导出成功！\n" + fn, Toast.LENGTH_SHORT);
			...
			MyToast.showToast(this, getString(R.string.an_error_occurred_export_failed), Toast.LENGTH_SHORT);
			finish();
		}
	}
		...
		switch (mt.getItemId()) {
			...
			case R.id.exp_gif_make:  //导出为动画
				...
				if (l <= 0 || l < m_Gif_Start)  {
					MyToast.showToast(this, getString(R.string.no_animation_for_forward_push_action), Toast.LENGTH_SHORT);
				...
				new AlertDialog.Builder(this, AlertDialog.THEME_HOLO_DARK).setTitle (getString(R.string.frame_interval))
						.setView (view3)
