...
public class myFileExplorerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (root.exists()) {
			...
			MyToast.showToast(this, getString(R.string.system_error_unable_to_perform_the_operation), Toast.LENGTH_SHORT);
			finish();
		}

		setTitle(getString(R.string.title_customized_position));

		//为ListView的列表项单击事件绑定监听器   | When clicking a directory in the list, show the files in that directory
		listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				//用户单击了文件，直接返回 = The user clicked on a file, return directly.
				if (currentFiles[position].isFile()) return;
				//获取单击文件夹下的所有文件 = Get all files within the clicked folder
		...
		dlg0.setTitle(R.string.alertTitle_attention).setMessage(R.string.exit_browsing_are_you_sure).setCancelable(false).setNegativeButton(getString(R.string.cancel), null)
...
//				}
	...
	private void inflateListView(File[] files) {
		...
		for (int i = 0; i < files.length; i++ ) {
			...
			//如果当前File是文件夹，使用文件夹图标，其它使用文件图标 = if the current File is a directory, use a folder icon; otherwise, use a file icon
