...
public class mySubmit extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        //设置标题栏标题，并开启标题栏的返回键
		setTitle(getString(R.string.title_submit_competition_solutions));
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(final Message msg) {
			if (msg.what == 1) {
				...
				builder.setTitle(getString(R.string.error)).setMessage (msg.obj.toString ()).setPositiveButton (getString(R.string.confirm), null);
		...
		try {
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				if (result.toLowerCase().indexOf("correct (for ") >= 0) {
					msg.what = 1;
					msg.obj = getString(R.string.submission_successful);
				} else if (result.toLowerCase().indexOf("not correct") >= 0) {
					msg.what = 2;
					msg.obj = getString(R.string.the_solution_is_incorrect);
				} else if (result.toLowerCase().indexOf("competition has ended") >= 0) {
					msg.what = 3;
					msg.obj = getString(R.string.competition_has_ended);
				} else if (result.toLowerCase().indexOf("not begin yet") >= 0) {
					msg.what = 4;
					msg.obj = getString(R.string.the_competition_has_not_yet_started);
				} else if (result.toLowerCase().indexOf("name cannot be empty") >= 0) {
					msg.what = 5;
					msg.obj = getString(R.string.please_enter_your_name);
				} else {
					msg.what = 6;
					msg.obj = getString(R.string.unknown_error);
				}
				handler.sendMessage(msg);
			} else {
				Message msg = Message.obtain();
				msg.what = 0;
				msg.obj = getString(R.string.network_error_please_contact_website_admin);
				handler.sendMessage(msg);
			}
			...
			Message msg = Message.obtain();
			msg.what = 0;
			msg.obj = getString(R.string.network_or_read_write_error);
			handler.sendMessage(msg);
		} catch (IOException e) {
			Message msg = Message.obtain();
			msg.what = 0;
